...
BOT_TOKEN="7022767978:AAHlj_lD2hghKgxV_fr7kst9pI49sIynzAc"

async def start1(update: Update, context):
    """Send a message when the command /start is issued."""
    await update.message.reply_text('Hi!')

async def whoami(update: Update, context):
    """Send a message when the command /start is issued."""
    await update.message.reply_text('Sou o bot do amor.\nDigite /lista para saber o que posso fazer por você :)')

async def help_command(update: Update, context):
    """Send a message when the command /help is issued."""
    await update.message.reply_text('0 Help!')

async def list(update: Update, context):
    """Send a message when the command /help is issued."""
    await update.message.reply_text('/start1\n/help\n/list\n/calculadora nome1 nome2\n/oraculo nome\n/quem')
...
async def oracle(update: Update, context):
    ...
    if valor % 3 == 0:
        ...
        await update.message.reply_text(f"{nome} vai ter que se virar! xD")
...
def main():
    ...
    application.add_handler(CommandHandler("quem", whoami))
    application.add_handler(CommandHandler("lista", list))
    application.add_handler(CommandHandler("calculadora", calcula_amor))
    application.add_handler(CommandHandler("oraculo", oracle))
